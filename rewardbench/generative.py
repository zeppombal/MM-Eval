...
prompt_v2 = (
    ...
    # removed tie option as , and \"[[C]]\ " for a tie
...
def format_judge_answers(question, answer_a, answer_b, multi_turn=False, model_modifier=None):
    ...
    if model_modifier == "prometheus":
        if multi_turn:
            raise ValueError(
                "Prometheus prompts do not support multi-turn prompts")
        else:
        ...
        if multi_turn:
            raise ValueError(
                "Offsetbias prompts do not support multi-turn prompts")
        else:
            system_prompt = ""
...
def run_judge_pair(question, answer_a, answer_b, model, multi_turn=False, model_modifier=None):
    ...
    if model in OPENAI_MODEL_LIST:
        ...
        judgment = chat_completion_gemini(
            model, text, temperature=0, max_tokens=4096)
        ...
        conv.append_message(conv.roles[1], None)
        conv.set_system_message(system_prompt)
        judgment = chat_completion_together(
            model, conv, temperature=0, max_tokens=2048)

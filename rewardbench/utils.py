...
# private until officially supported
...
def save_to_hub(
    ...
    if not local_only:
        scores_url = api.upload_file(
            ...
            repo_id=EVAL_REPO if not debug else "ai2-adapt-dev/herm-debug",
            repo_type="dataset",
...
def load_preference_dataset(
    ...
    assert features[
        "chosen"].dtype == "string", f"chosen is wrong type (should be string): {features['chosen']}"
    assert features[
        "rejected"].dtype == "string", f"rejected is wrong type (should be string): {features['rejected']}"
...
def load_eval_dataset(
    ...
    Args:
        custom_dataset_path: path to a .jsonl file with the same format as the RewardBench datasets.
        ...
        if core_set:
            ...
            for subset_name, subdataset in raw_dataset.items():
                ...
                if "subset" in subdataset.column_names:
                    subdataset = subdataset.rename_column(
                        "subset", "subsubset")
            ...
            if core_set:
                if isinstance(example["prompt"], list) and len(example["prompt"]) > 0:
                    if isinstance(example['prompt'][0], str):
                        ...
                        example['text_chosen'] = messages + \
                            [{"role": "assistant", "content": example["chosen"]}]
                        example['text_rejected'] = messages + \
                            [{"role": "assistant", "content": example["rejected"]}]
                else:
                    example["text_chosen"] = [
...
def load_bon_dataset(
    ...
    alpaca_eval = load_dataset(
        "ai2-adapt-dev/HERM_BoN_candidates", "alpaca_eval")
    ...
    merged_mt_bench = concatenate_datasets(
        [mt_bench["zephyr"], mt_bench["tulu"]])
    ...
    merged_alpaca_eval = merged_alpaca_eval.rename_column(
        "dataset", "dataset_details")
    merged_mt_bench = merged_mt_bench.rename_column(
        "category", "dataset_details")
            ...
            for i, (line) in enumerate(example["prompt"]):
                ...
                if (i + 1) % 2 == 1:
                    dialogue_template.messages.append(
                        [dialogue_template.roles[0], p])
                else:
                    dialogue_template.messages.append(
                        [dialogue_template.roles[1], p])
            # assert that the last message before this is user

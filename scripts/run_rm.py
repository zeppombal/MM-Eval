...
from model import CohereForSequenceClassification
...
def main():
    ...
    if 'aya' in args.model:
        config = REWARD_MODEL_CONFIG["custom-cohere"]
    else:
    ...
    ############################
        ...
        for step, batch in enumerate(tqdm(dataloader, desc="RM batch steps")):
            ...
            if model_type == "Custom Classifier":
                ...
                [
                    results.append(
                        1) if chosen > rejected else results.append(0)
    ...
    results_grouped["chat_template"] = (
        args.chat_template if not check_tokenizer_chat_template(
    ...
    if args.custom_dataset_path:
        dataset_name = args.custom_dataset_path.split("/")[-1].split(".")[0]
    else:
        dataset_name = "rewardbench"

    sub_path = f"eval-set/{dataset_name}/" if not args.pref_sets else f"pref-sets/{dataset_name}/"
    ...
    if not model_type == "Custom Classifier":  # custom classifiers do not return scores
        ...
        sub_path_scores = f"eval-set-scores/{dataset_name}/" if not args.pref_sets else f"pref-sets-scores/{dataset_name}"

        scores_url = save_to_hub(
            scores_dict, args.model, sub_path_scores, args.debug, local_only=True)
        logger.info(
            f"Uploading chosen-rejected text with scores to {scores_url}")
    else:
        logger.info(
